...
SELECT 
    ...
    COUNT(*) FILTER (WHERE current_state = 'unknown'::learning_state) as unknown_count,
    COUNT(*) FILTER (WHERE current_state = 'learning'::learning_state) as learning_count,
    COUNT(*) FILTER (WHERE current_state = 'known'::learning_state) as known_count,
    COUNT(*) FILTER (WHERE current_state = 'memorized'::learning_state) as memorized_count,
    COUNT(*) FILTER (WHERE current_state = 'long_term'::learning_state) as long_term_count,
    ROUND(CAST(AVG(total_score) AS NUMERIC), 2) as average_total_score,
