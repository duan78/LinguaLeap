...
-- Add SRS-specific columns to word_progress table
ALTER TABLE word_progress
ADD COLUMN IF NOT EXISTS state TEXT NOT NULL DEFAULT 'new' 
  CHECK (state IN ('new', 'learning', 'known', 'mastered', 'long-term')),
...
-- Create index for next review date queries
CREATE INDEX IF NOT EXISTS idx_word_progress_next_review 
ON word_progress(next_review);

-- Create word_progress_stats view with consistent column names
