...
word_stats AS (
    SELECT 
        ...
        calculate_word_score(wp.mastery_level, wp.correct_streak) as score,
        wp.last_reviewed,
        wp.next_review
    FROM flashcards f
...
WITH user_stats AS (
    SELECT 
        ...
        -- Use the same average_total_score from learning_progress_summary
        CAST(AVG(kls.total_score) AS NUMERIC(10,2)) as average_score,
        -- Calculate success rate
        CAST(
            CASE 
                WHEN COUNT(*) > 0 
                THEN COUNT(CASE WHEN kls.current_state != 'unknown' THEN 1 END)::NUMERIC / COUNT(*) * 100
                ELSE 0 
            END 
        ...
        COUNT(DISTINCT CASE WHEN kls.current_state = 'memorized' THEN kls.flashcard_id END) as mastered_count,
        COUNT(DISTINCT CASE WHEN kls.current_state = 'long_term' THEN kls.flashcard_id END) as long_term_count
...
RETURNS TRIGGER AS $$
BEGIN
    NEW.score = calculate_word_score(NEW.mastery_level, NEW.correct_streak);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER word_score_update_trigger
    BEFORE INSERT OR UPDATE OF mastery_level, correct_streak
    ON word_progress
    FOR EACH ROW
