...
-- Create word_progress_stats view with improved tracking
...
WITH user_stats AS (
    SELECT 
        ...
        SUM(CASE WHEN wp.mastery_level > 0 THEN 1 ELSE 0 END) as total_progress,
        COUNT(DISTINCT CASE 
...
SELECT 
    ...
    CASE 
        WHEN us.total_words > 0 THEN 
            ROUND((us.total_progress::NUMERIC / (us.total_words * 5)) * 100)
        ELSE 0 
    END as review_success
FROM user_stats us
LEFT JOIN streak_calc sc ON us.user_id = sc.user_id;

-- Add new columns to word_progress if they don't exist
