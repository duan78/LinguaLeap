...
-- Create a function to determine mastery status
...
BEGIN
    RETURN 
        -- Word is considered mastered if:
        -- 1. Mastery level is 5 or higher
        -- 2. Has been reviewed at least 5 times
        -- 3. Has a correct streak of at least 3
        mastery_level >= 5 AND 
        review_count >= 5 AND 
        correct_streak >= 3;
END;
...
-- Create word_progress_stats view with fixed mastery tracking
...
WITH user_stats AS (
    SELECT 
        ...
        COUNT(DISTINCT CASE 
            WHEN is_word_mastered(wp.mastery_level, wp.correct_streak, wp.review_count)
...
BEGIN
    ...
    IF NEW.correct_streak >= 5 AND NEW.review_count >= 5 THEN
        NEW.mastery_level := 5; -- Mastered
    ELSIF NEW.correct_streak >= 3 AND NEW.review_count >= 3 THEN
        NEW.mastery_level := 4; -- Almost mastered
    ELSIF NEW.correct_streak >= 2 THEN
        NEW.mastery_level := 3; -- Well known
    ELSIF NEW.correct_streak >= 1 THEN
        NEW.mastery_level := 2; -- Learning
    ELSIF NEW.review_count > 0 THEN
        NEW.mastery_level := 1; -- Started learning
    ELSE
