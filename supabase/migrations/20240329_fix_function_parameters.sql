...
DROP FUNCTION IF EXISTS update_word_progress();

-- Drop all versions of calculate_word_score function
DROP FUNCTION IF EXISTS calculate_word_score(integer, integer);
DROP FUNCTION IF EXISTS calculate_word_score(integer, integer, integer);

-- Create new version of calculate_word_score with renamed parameters
...
-- Recreate the progress update trigger function
CREATE OR REPLACE FUNCTION update_word_progress()
RETURNS TRIGGER AS $$
BEGIN
    -- Calculate new score using the renamed function
...
-- Recreate views that depend on these functions
