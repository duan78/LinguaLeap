...
-- Create a function to calculate the score consistently
...
-- Create word_progress_stats view with the same score calculation
...
WITH user_stats AS (
    SELECT 
        ...
        -- Calculate average score using the same function
        CAST(AVG(
            CASE WHEN wp.review_count > 0 
            THEN calculate_word_score(wp.mastery_level, wp.correct_streak)
            ELSE 0 END
